...
The easiest solutions are to either use 7-Zip File Manager or just <code class="language-bash">binwalk -eM flag.rar</code> on Linux (```-e``` means extract, ```-M``` is basically recursive), which results in a final file, flag.gif.  

Use a hex editor and Ctrl+F for "INTIGRITI" or do <code class="language-bash">grep "INTIGRITI" flag.gif</code> on Linux to get the flag!  
